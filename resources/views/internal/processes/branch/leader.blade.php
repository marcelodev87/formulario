...
@section('title', 'Dirigente da filial | Processo #' . $process->id)
    ...
    <div class="card space-y-3">
        <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            ...
            <div class="rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-600">
                <p><span class="font-semibold text-slate-800">Filial:</span> {{ $location->name ?? 'Sem identificacao' }}</p>
                <p><span class="font-semibold text-slate-800">Instituicao:</span> {{ $institution->name }}</p>
            </div>
        </div>
    </div>

    <form method="POST" action="{{ route('etika.processes.branch.leader.update', $process) }}" class="card space-y-6">
        @csrf
        @method('PUT')
        ...
        <section class="space-y-4">
            ...
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <div class="space-y-1">
                    ...
                    <input id="cpf" name="cpf" type="text" class="form-control" value="{{ old('cpf', $leader->cpf ?? '') }}" placeholder="000.000.000-00" data-mask="cpf" required>
                </div>
                <div class="space-y-1">
        ...
        <section class="space-y-4">
            ...
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <div class="space-y-1">
                    ...
                    <input id="phone" name="phone" type="text" class="form-control" value="{{ old('phone', $leader->phone ?? '') }}" placeholder="(00) 00000-0000" data-mask="phone" required>
                </div>
            </div>
        </section>
        ...
        <section class="space-y-4">
            ...
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <div class="space-y-1">
                    ...
                    <input id="cep" name="cep" type="text" class="form-control" value="{{ old('cep', $leader->cep ?? '') }}" placeholder="00000-000" data-mask="cep" required>
                </div>
            </div>
        </section>
