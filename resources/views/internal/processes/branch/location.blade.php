...
@section('title', 'Dados da filial | Processo #' . $process->id)

@section('content')
@php
    $isNewLocation = !$location->exists;
    $tenureType = old('tenure_type', optional($property)->tenure_type ?? 'own');
@endphp
<div class="space-y-8">
    <div class="card space-y-3">
        ...
        <div class="flex flex-wrap items-center gap-3 text-xs text-slate-500">
            ...
            <span>Atualizado em {{ $process->updated_at?->timezone('America/Sao_Paulo')->format('d/m/Y H:i') }}</span>
        </div>
    </div>

    <form method="POST" action="{{ route('etika.processes.branch.location.update', $process) }}" class="card space-y-6">
        @csrf
        @method('PUT')
        ...
        <section id="property-section" class="space-y-4">
            ...
            <div class="grid gap-4 md:grid-cols-3">
                ...
                <div class="space-y-1">
                    <label class="form-label" for="built_area_sqm">Area construida (m²)</label>
                ...
                <div class="space-y-1">
                    <label class="form-label" for="land_area_sqm">Area total do terreno (m²)</label>
            ...
            <div class="grid gap-4 md:grid-cols-2">
                <div class="space-y-1">
                    ...
                    <label class="flex items-center gap-2 text-sm">
                        <input type="radio" name="tenure_type" value="own" {{ $tenureType === 'own' ? 'checked' : '' }} required>
                        Proprio
                    </label>
                    <label class="flex items-center gap-2 text-sm">
                        <input type="radio" name="tenure_type" value="rented" {{ $tenureType === 'rented' ? 'checked' : '' }}>
                        Alugado
                    </label>
                </div>
                <div class="space-y-1">
                ...
                <div class="space-y-1 md:col-span-2">
                    <label class="form-label" for="property_use">Uso do imovel</label>
                    <input id="property_use" name="property_use" type="text" class="form-control" value="{{ old('property_use', optional($property)->property_use) }}" placeholder="Cultos, escola dominical..." />
                </div>
                <div class="space-y-1 md:col-span-2">
