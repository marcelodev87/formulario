...
@php
    ...
    $redirectProcessId = request('process_id');

    if (!is_null($redirectProcessId) && !is_numeric($redirectProcessId)) {

        $redirectProcessId = null;

    } elseif (!is_null($redirectProcessId)) {

        $redirectProcessId = (int) $redirectProcessId;

    }

    $cancelUrl = route('dashboard');

    if ($redirectTo === \App\Models\Process::TYPE_BOARD_ELECTION_MINUTES_REGISTRATION && $redirectProcessId) {

        $cancelUrl = route('processes.board_election.members', ['process' => $redirectProcessId]);
...
<div class="card space-y-8">
    ...
    <form method="POST" action="{{ route('members.update', $member) }}" class="space-y-6">
        ...
        @if(!is_null($redirectProcessId))

            <input type="hidden" name="process_id" value="{{ $redirectProcessId }}">
            ...
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <div class="space-y-1">
                    ...
                    <select id="gender" name="gender" class="form-control" required>
                        ...
                        <option value="Feminino" {{ old('gender', $member->gender) === 'Feminino' ? 'selected' : '' }}>Feminino</option>
                ...
                <div class="space-y-1">
                    ...
                    <select id="marital_status" name="marital_status" class="form-control" required>
                        ...
                        <option value="Divorciado(a)" {{ old('marital_status', $member->marital_status) === 'Divorciado(a)' ? 'selected' : '' }}>Divorciado(a)</option>

                        <option value="Casado(a)" {{ old('marital_status', $member->marital_status) === 'Casado(a)' ? 'selected' : '' }}>Casado(a)</option>

                        <option value="Solteiro(a)" {{ old('marital_status', $member->marital_status) === 'Solteiro(a)' ? 'selected' : '' }}>Solteiro(a)</option>

                        <option value="Vi�vo(a)" {{ old('marital_status', $member->marital_status) === 'Vi�vo(a)' ? 'selected' : '' }}>Vi�vo(a)</option>
